...
4. **Given** 應用程式已載入，**When** 使用者查看畫面底部，**Then** 顯示深藍色圓形浮動按鈕（64×64px）帶有白色 + 號圖示
...
- **FR-005**: 系統**必須 (MUST)** 在卡片底部顯示一個圓形浮動按鈕（FloatingActionButton）
- **FR-006**: 使用者點擊浮動按鈕時，系統**必須 (MUST)** 將計數值遞增 1
...
- **FR-014**: 浮動按鈕**必須 (MUST)** 使用深藍色背景（#030213）
- **FR-015**: 浮動按鈕**必須 (MUST)** 顯示白色 + 號圖示（#FFFFFF）
- **FR-016**: 浮動按鈕**必須 (MUST)** 為圓形（64×64px）
- **FR-017**: 浮動按鈕**必須 (MUST)** 具有雙層陰影效果（參考設計規格）

#### 可存取性與測試

- **FR-018**: 標題元件**必須 (MUST)** 具有唯一識別符號 `Key('counter_title')`
- **FR-019**: 計數顯示元件**必須 (MUST)** 具有唯一識別符號 `Key('counter_display')`
- **FR-020**: 浮動按鈕**必須 (MUST)** 具有唯一識別符號 `Key('increment_button')`
- **FR-021**: 計數顯示**必須 (MUST)** 包含語義標籤（semanticsLabel）以支援螢幕閱讀器
- **FR-022**: 浮動按鈕**必須 (MUST)** 包含工具提示（tooltip）說明其功能
